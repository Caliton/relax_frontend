    ...
    >
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td auto-width :props="props" key="hiringDate"> {{ props.row.hiringDate | moment('DD-MM-YYYY')}} </q-td>
          <q-td auto-width :props="props" key="birthDay"> {{props.row.birthDay | moment('DD-MM-YYYY')}} </q-td>

          <q-td auto-width>
            <q-btn size="sm" flat class="q-ma-xs bg-white" color="light-blue" round dense icon="eva-edit-outline" @click="openEdit(props.row)"/>
            <q-btn size="sm" flat class="q-ma-xs bg-white" color="red" round dense icon="eva-trash-2-outline" @click="openDelete(props.row)"/>
          </q-td>
        </q-tr>
      </template>


      <div v-if="false" slot="bottom" slot-scope="props" class="flex flex-center" style="width: 100%;">
    ...
    <q-dialog v-model="showDelete" persistent>
      <q-card>
        ...
        <q-card-actions align="right">
          ...
          <q-btn flat no-caps label="Sim, não quero mais!" color="red" @click="deletePerson" v-close-popup />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      visibleColumns: ["status", "firstName", "lastName", "hiringDate", "birthDay"],
      columns: [
        { align: 'left', name: "id", label: "id", field: "id", sortable: true},
        { align: 'left', name: "status", label: "Status", field: "status", sortable: true, style: 'width: 10px', headerStyle: 'width: 50px'},
        { align: 'left', name: "firstName", label: "Nome", field: "firstName", sortable: true},
        { align: 'left', name: "lastName", label: "Sobrenome", field: "lastName", sortable: true},
        { align: 'left', name: "hiringDate", label: "Data de admissão", field: "hiringDate", sortable: true},
        { align: 'left', name: "birthDay", label: "Aniversário", field: "birthDay", sortable: true}
      ],
      data: []
    };
  },
  ...
  methods: {
    ...
    async onRequest(props) {
      ...
      console.log('Resultado: ', returnedData.data);
    ...
    async deletePerson () {
      try {
        ...
        await this.$axios.delete(`${'/person'.replace('{id}', this.evaluationFocus.id)}/`)
